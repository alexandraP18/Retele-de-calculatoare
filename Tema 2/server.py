    ...
    try:
        ...
        if comanda == 'CONNECT':
            ...
            if adresa_client not in clienti_conectati:
                ...
                try:
                    ...
                    if id_mesaj not in mesaje:
                        ...
                        raspuns = f"EROARE: Nu poti sterge mesajul cu ID-ul {id_mesaj} deoarece nu esti autorul mesajului."
                    else:
                        del mesaje[id_mesaj]
                        raspuns = f"OK: Mesajul cu ID-ul {id_mesaj} a fost sters cu succes! :)"
            ...
            if adresa_client not in clienti_conectati:
                ...
                raspuns = "OK: Nu exista mesaje publicate momentan"
            else:
                linii = ["OK: Lista mesaje:"]
                for mid, mdata in mesaje.items():
                    linii.append(f"[ID={mid}]: {mdata['text']} (autor: {mdata['autor']})")
                    raspuns = "\n".join(linii)
