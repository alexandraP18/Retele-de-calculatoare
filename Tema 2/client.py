...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Nu esti conectat la server. Foloseste comanda CONNECT inainte.")
            continue
        if not argumente:
            print("EROARE: Trebuie sa furnizezi un mesaj pentru PUBLISH. Ex: PUBLISH saluuutt!! :D")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            ...
            continue
        if not argumente:
            print("EROARE: Trebuie sa furnizezi un ID. Ex: DELETE 3")
            continue
        try:
            int(argumente)
        except ValueError:
            print(f"EROARE: {argumente} nu este un nr. intreg valid. Ex: DELETE 3")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
